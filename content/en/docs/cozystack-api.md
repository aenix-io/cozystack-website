...
You can list of available resources using `kubectl`:
...
Then request specific kind in tenant namespace:
...
status:
  ...
  - lastTransitionTime: "2024-12-10T09:53:32Z"
    message: Helm install succeeded for release tenant-test/postgres-test.v1 with
      chart postgres@0.7.1
...
You can can use this resource as example to create similar service through API.

Just save the output into file, replace `name` and some parameters. And use `kubectl` to create a new postgres instance:
...
#### How to use Terraform with Cozystack API

Cozystack supports Terraform. The default `kubernetes` provider can be used to create resources in Cozystack API.
...
And your new Postgres cluster will be deployed
